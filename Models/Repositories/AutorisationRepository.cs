...
namespace ActivityLogger.Models.Repositories
{
    public class AutorisationRepository: IAutorisationRepository
    {
        ...
        private List<Autorisation> GetAutorisationList()
        {
            ...
            {
                autorisationList = db.Query<Autorisation>("SELECT * FROM Autorization").ToList();
            }
        ...
        public Autorisation Get(string login, string password)
        {
            ...
            using (IDbConnection db = new SqlConnection(connectionString))
            {
                autorisation = db.Query<Autorisation>("SELECT * FROM Autorization WHERE Login = @login AND Password = @password", new { login, password}).FirstOrDefault();
            }
            return autorisation;
        }
        public Autorisation Get(int id)
        {
            ...
            using (IDbConnection db = new SqlConnection(connectionString))
            {
                autorisation = db.Query<Autorisation>("SELECT * FROM Autorization WHERE Id = @id", new { id }).FirstOrDefault();
            }
            return autorisation;
        }
